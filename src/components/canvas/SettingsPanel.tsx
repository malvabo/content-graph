...
export default function SettingsPanel() {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', background: 'var(--color-bg)', overflow: 'hidden' }}>
      {/* Sidebar nav */}
      <nav className="hidden md:flex" style={{ width: 200, flexShrink: 0, borderRight: '1px solid var(--color-border-subtle)', background: 'var(--color-bg-card)', padding: '24px 20px', flexDirection: 'column', overflowY: 'auto' }}>
          {Object.entries(groups).map(([group, items], gi) => (
            <div key={group} style={{ marginTop: gi > 0 ? 24 : 0 }}>
              <div style={{ fontSize: 14, fontWeight: 500, fontFamily: 'var(--font-sans)', color: 'var(--color-text-tertiary)', marginBottom: 8 }}>{group}</div>
              {items.map(s => {
                const on = active === s.id;
                return (
                  <button key={s.id} onClick={() => setActive(s.id)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-sans)', fontSize: 14, fontWeight: on ? 500 : 400, color: on ? 'var(--color-accent)' : 'var(--color-text-primary)', transition: 'color 100ms', textAlign: 'left' }}>
