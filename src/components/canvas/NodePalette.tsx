  ...
  return (
    <div draggable onDragStart={onDragStart}
      ...
      <div className="min-w-0">
        ...
        <div style={{ font: '400 14px/18px var(--font-sans)', color: 'var(--cg-ink-2)' }} className="truncate">{def.description}</div>
      </div>
    </div>
  );
}
      ...
      {PALETTE_ORDER.map((cat) => {
        ...
        return (
          <div key={cat} className="px-2 mb-3">
            {isAdvanced ? (
              <button className="flex items-center gap-1 px-1 mb-1.5" style={{ font: '500 14px/1 var(--font-sans)', color: 'var(--cg-ink-3)' }} onClick={() => setAdvancedOpen(!advancedOpen)}>
                <span style={{ fontSize: 14 }}>{advancedOpen ? '▼' : '▶'}</span>
                {CATEGORY_LABELS[cat]}
              </button>
            ) : (
              <div className="px-1 mb-1.5" style={{ font: '500 14px/1 var(--font-sans)', color: 'var(--cg-ink-3)' }}>{CATEGORY_LABELS[cat]}</div>
            )}
