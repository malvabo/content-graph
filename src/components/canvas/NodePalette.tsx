  ...
  return (
    <div draggable onDragStart={onDragStart} onClick={onClick}
      ...
      <div className="min-w-0">
        <div style={{ font: '500 14px/18px var(--font-sans)', color: 'var(--cg-ink)' }} className="truncate">{def.label}</div>
          ...
          {PALETTE_ORDER.map((cat) => {
            ...
            return (
              <div key={cat} className="px-2 mb-3">
                {isAdvanced ? (
                  <button className="flex items-center gap-1.5 px-3 mb-1.5" style={{ font: '500 12px/1 var(--font-sans)', color: '#6d6d6d' }} onClick={() => setAdvancedOpen(!advancedOpen)}>
                    <span style={{ fontSize: 10 }}>{advancedOpen ? '▼' : '▶'}</span>
                    {CATEGORY_LABELS[cat]}
                  </button>
                ) : (
                  <div className="px-3 mb-1.5" style={{ font: '500 12px/1 var(--font-sans)', color: '#6d6d6d' }}>{CATEGORY_LABELS[cat]}</div>
                )}
