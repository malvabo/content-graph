...
function NavItem({ icon, label, active, onClick, ariaLabel, ariaPressed }: { icon: ReactNode; label: string; active?: boolean; onClick: () => void; ariaLabel?: string; ariaPressed?: boolean }) {
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} aria-label={ariaLabel ?? label} aria-pressed={ariaPressed}
      className="nav-item"
      style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '7px 10px', borderRadius: 18, background: active ? 'var(--color-nav-item-active)' : hover ? 'var(--color-nav-item-hover)' : 'transparent', color: 'var(--color-nav-text)', boxShadow: active ? '0 1px 2px 0 rgba(0,0,0,0.04)' : 'none', transition: 'none', justifyContent: 'flex-start', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-sans)' }}>
  ...
  return (
    <>
      ...
      <nav aria-label="Main navigation" className="icon-nav"
        style={{ width: 200, height: 'calc(100% - 16px)', margin: '8px', display: 'flex', flexDirection: 'column', alignItems: 'stretch', padding: '12px 4px', gap: 2, flexShrink: 0, background: 'var(--color-nav-bg)', border: 'none', borderRadius: 12, boxShadow: 'var(--shadow-nav)', position: 'relative', zIndex: 2 }}>
