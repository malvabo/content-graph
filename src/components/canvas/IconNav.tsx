...
function NavItem({ icon, label, active, onClick }: { icon: ReactNode; label: string; active: boolean; onClick: () => void }) {
  ...
  return (
    <div className="relative w-full flex justify-center">
      {/* Left accent bar */}
      {active && (
        <div className="absolute left-0 top-1 bottom-1 w-[3px] rounded-r-full" style={{ background: 'var(--color-accent)' }} />
      )}
      <button
        onClick={onClick}
        ...
        style={{
          ...
          color: active ? 'var(--color-accent-subtle)' : 'var(--color-text-tertiary)',
        }}
      >
        {icon}
      </button>
...
export default function IconNav({ activeView, onViewChange }: Props) {
  return (
    <nav aria-label="Main navigation" className="w-[52px] shrink-0 flex flex-col items-center py-3 gap-1" style={{ background: 'var(--color-bg-card)', boxShadow: '1px 0 0 0 var(--color-border-subtle)' }}>
      {/* Logo */}
      <div className="w-8 h-8 rounded-lg flex items-center justify-center mb-2" style={{ background: 'var(--color-bg-surface)', color: 'var(--color-text-primary)', fontWeight: 600, fontSize: 'var(--text-xs)', fontFamily: 'var(--font-sans)', userSelect: 'none' }}>
        up
      </div>
