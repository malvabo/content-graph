...
import '../index.css';
import { OutputModal, ImageModal, ModalShell, ModalHeader, ModalFooter } from '../components/modals/Modals';
...
function TextTrigger() { const [o, setO] = useState(false); return <><button className="btn btn-primary" onClick={() => setO(true)}>Open Text Modal</button>{o && <OutputModal title="LinkedIn Post" text={sampleText} wordCount={sampleText.split(/\s+/).length} onClose={() => setO(false)} />}</>; }
